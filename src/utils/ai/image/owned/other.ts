...
/**
 * 智谱AI CogView 图像生成
 * API文档: https://docs.bigmodel.cn/api-reference/模型-api/图像生成
 * 
 * 支持的size参数:
 * - 1024x1024（默认）
 * - 768x1344
...
async function generateZhipuImage(input: ImageConfig, config: AIConfig): Promise<string> {
  ...
  // 智谱size参数映射 - 必须严格使用支持的7种尺寸
  if (input.size) {
    const zhipuSizeMap: Record<string, string> = {
      "1K": "1024x1024",
      "2K": "1440x720",  // 智谱不支持2048，使用最接近的
      "4K": "1440x720",  // 智谱最大支持1440x720
      "1024x1024": "1024x1024",
      "768x1344": "768x1344",
      "864x1152": "864x1152",
      "1344x768": "1344x768",
    ...
    // 如果size在映射表中，使用映射值；否则默认1024x1024
    if (zhipuSizeMap[input.size]) {
      body.size = zhipuSizeMap[input.size];
    } else {
      console.warn(`%c 智谱AI: 不支持的size "${input.size}"，使用默认1024x1024`, "background:#ffaa00");
      body.size = "1024x1024";
    }
  }
  
  // quality参数: standard(默认) 或 hd
  if (input.quality) {
  ...
  console.log("%c 智谱AI请求体", "background:#33a5ff", body);
...
async function generateModelScopeImage(input: ImageConfig, config: AIConfig): Promise<string> {
  ...
  // ModelScope size映射
  if (input.size) {
    const msSizeMap: Record<string, string> = {
      "1K": "1024x1024",
      "2K": "2048x2048",
      "4K": "4096x4096",
    };
    body.size = msSizeMap[input.size] || input.size;
  }
  ...
  console.log("%c ModelScope请求体", "background:#33a5ff", body);
